...
public class Main {
    ...
    private static void automatic(LinkedListContainer<TradingPoint> listContainer) {
        ...
        //достаем колекцию с файла
        LinkedListContainer<TradingPoint> tradingPoints = listContainer.loadFromFile("safe.bin");
        //флаг для проверки
        boolean isValid = true;
        //проверяем колекцию с файла
        for (TradingPoint tp : tradingPoints) {
            //есл значения неправельние виводим сообшение и присваиваем флажку другое значение
            if (!tp.validate()) {
                System.out.println("This file provide incorrect values");
                isValid = false;
            }
        }
        //если флаг = true то присваиваем нашему list значение из файла
        if (isValid) {
    ...
    public static void main(String[] args) {
        ...
        //проверка на наличее елемента -auto в args[] - args хранит такие переменние которие вводиш
        ...
        while (isEnd) {
            ...
            switch (numberImp) {
                ...
                case ("4"):
                    ...
                    //создаем екземпляр Trading Point со значениями введенних с конссоли
                    TradingPoint someThing = new TradingPoint(tpName, tpAddress, List.of(tpPhones), "someThing",
                            LocalDateTime.of(2004, 1, 1, 8, 30),
                            LocalDateTime.of(2010, 2, 5, 19, 30));
                    //проверяем на корректность данних
